...
module PaymentStatuses
  def update_expired_signup_status
    unless Payment.where('payment_description = ? AND student_id = ?', 'Inscripción', student_id).empty?
  ...
  def update_expired_fee_status
    unless Payment.where('payment_description = ? AND student_id = ?', 'Cuota', student_id).empty?
